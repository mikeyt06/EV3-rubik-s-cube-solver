...
class Rubiks(Robot):
    ...
    def rotate_cube_1(self):
        log.info("rotate_cube_1: Rotating cube 1 time in positive direction")
        self.rotate_cube(1, 1)

    def rotate_cube_2(self):
        log.info("rotate_cube_2: Rotating cube 2 times in positive direction")
        self.rotate_cube(1, 2)

    def rotate_cube_3(self):
        log.info("rotate_cube_3: Rotating cube 1 time in negative direction")
        self.rotate_cube(-1, 1)

    def rotate_cube_blocked(self, direction, nb):
        log.info("rotate_cube_blocked: Starting blocked rotation, direction={}, times={}".format(direction, nb))

        # Move the arm down to hold the block in place
        if self.mot_push.get_position() < Rubiks.hold_cube_pos:
            log.info("rotate_cube_blocked: Arm position {} < hold_cube_pos {}, moving arm to hold position.".format(self.mot_push.get_position(), Rubiks.hold_cube_pos))
            self.mot_push.goto_position(Rubiks.hold_cube_pos, 300, stop_mode='hold')
            self.mot_push.wait_for_start()
            self.mot_push.wait_for_stop()
            self.mot_push.stop()
            log.info("rotate_cube_blocked: Arm moved to hold position")

        # This depends a lot on Rubiks.rotate_speed
        OVERROTATE = 18
        current_position = self.mot_rotate.get_position()
        final_dest = 135 * round((current_position + (270 * direction * nb)) / 135.0)
        temp_dest = final_dest + (OVERROTATE * direction)
        log.info("rotate_cube_blocked: Current position={}, final_dest={}, temp_dest={}".format(current_position, final_dest, temp_dest))
        ...
        self.mot_rotate.goto_position(
            ...
            stop_mode='hold')
        self.mot_rotate.wait_for_stop()
        log.info("rotate_cube_blocked: Rotated to temporary position {}".format(temp_dest))
        ...
        self.mot_rotate.wait_for_stop()
        self.mot_rotate.stop()
        log.info("rotate_cube_blocked: Rotated to final position {}".format(final_dest))

    def rotate_cube_blocked_1(self):
        log.info("rotate_cube_blocked_1: Rotating blocked cube 1 time in positive direction")
        self.rotate_cube_blocked(1, 1)

    def rotate_cube_blocked_2(self):
        log.info("rotate_cube_blocked_2: Rotating blocked cube 2 times in positive direction")
        self.rotate_cube_blocked(1, 2)

    def rotate_cube_blocked_3(self):
        log.info("rotate_cube_blocked_3: Rotating blocked cube 1 time in negative direction")
        self.rotate_cube_blocked(-1, 1)
    ...
    def push_arm_away(self):
        ...
        """
        log.info("push_arm_away: Moving the flipper arm out of the way")
        self.mot_push.goto_position(0, 400)
        self.mot_push.wait_for_stop()
        self.mot_push.stop()
        log.info("push_arm_away: Flipper arm moved to position 0")


    def flip(self):
        log.info("flip: Starting cube flip")
        if self.shutdown_flag:
            log.warning("flip: Shutdown flag is set, exiting flip function")
            return

        current_position = self.mot_push.get_position()
        log.info("flip: Current arm position is {}".format(current_position))
        ...
        if (current_position <= Rubiks.hold_cube_pos - 10 or
            ...
            log.info("flip: Position {} not in range, moving to hold position {}".format(current_position, Rubiks.hold_cube_pos))
            self.mot_push.goto_position(Rubiks.hold_cube_pos, 400)
            self.mot_push.wait_for_stop()
            self.mot_push.stop()
            log.info("flip: Moved to hold position {}".format(Rubiks.hold_cube_pos))

        # Grab the cube and pull back further (try 220 instead of 180)
        log.info("flip: Pulling the arm back further to grab the cube")
        self.mot_push.goto_position(165, 800) 
        self.mot_push.wait_for_stop()
        self.mot_push.stop()
        log.info("flip: Arm pulled back further to position 220")

        # At this point the cube is at an angle, push it forward to
        # drop it back down in the turntable
        log.info("flip: Pushing the arm forward to drop the cube back at position {}".format(Rubiks.hold_cube_pos))
        self.mot_push.goto_position(Rubiks.hold_cube_pos, 580)
        self.mot_push.wait_for_stop()
        self.mot_push.stop()
        log.info("flip: Arm pushed forward to position {}".format(Rubiks.hold_cube_pos))

        transformation = [2, 4, 1, 3, 0, 5]
        log.info("flip: Applying transformation {}".format(transformation))
        self.apply_transformation(transformation)
        log.info("flip: Flip completed")
    ...
    def put_arm_middle(self):
        self.mot_bras.goto_position(-760, 1200, stop_mode='hold')
        self.mot_bras.wait_for_stop()
    ...
    def put_arm_edge(self, i):
        ...
        #    diff = 0
        diff = 0
        self.mot_bras.goto_position(-645 - diff, 1200, stop_mode='hold')
        self.mot_bras.wait_for_stop()
    ...
    def scan_face(self, last_face=False):
        ...
        self.mot_rotate.reset()
        self.mot_rotate.rotate_position(1080, 120, 0, 0, 'on', stop_mode='hold')
